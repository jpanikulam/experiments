...
cv::Mat compute_hessians(const cv::Mat &image) {
  ...
  for (int row = 1; row < image.rows - 1; ++row) {
    for (int col = 1; col < image.cols - 1; ++col) {
      ...
      // hessian.at<cv::Vec3f>(row, col) = cv::Vec3f(ixx, ixy, iyy);
      if (eigvals(0) < 1.2 * eigvals(1) && eigvals(1) > 10) {
        // hessian.at<float>(row, col) = eigvals(0) * eigvals(1);
        hessian.at<float>(row, col) = 1.0;
      }
    }
  }
...
void run() {
  ...
  for (int k = 1; k < 45; ++k) {
    ...
    const cv::Mat hess = compute_hessians(rendered_image);
