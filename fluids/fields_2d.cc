...
std::array<Eigen::MatrixXd, 2> compute_gradient(const Eigen::MatrixXd& potential_field, const double dx) {
  ...
  out[0].middleRows(1, rows - 1) = (potential_field.bottomRows(rows - 1) - potential_field.topRows(rows - 1)) * inv_dx;

  out[1].middleCols(1, cols - 1) = (potential_field.rightCols(cols - 1) - potential_field.leftCols(cols - 1)) * inv_dx;
...
Eigen::MatrixXd compute_divergence(const std::array<Eigen::MatrixXd, 2>& vector_field, const double dx) {
  ...
  const double inv_dx = 1.0 / (2.0 * dx);
  result.middleRows(1, rows - 1) += (u.bottomRows(rows - 1) - u.topRows(rows - 1)) * inv_dx;

  result.middleCols(1, cols - 1) += (v.rightCols(cols - 1) - v.leftCols(cols - 1)) * inv_dx;
