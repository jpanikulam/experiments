...
StateDelta StateDelta::from_vector(const VecNd<8> &in_vec) {
  ...
  const StateDelta out = StateDelta{(in_vec[0]), (in_vec[1]), anon_b09bdd, anon_416527};
  return out;
}
...
State operator+(const State &a, const StateDelta &grp_b) {
  const State out =
      State{((a.x_vel) + (grp_b.x_vel_error)), ((a.phi) + (grp_b.phi_error)),
...
VecNd<8> State::compute_delta(const State &a, const State &b) {
  ...
  const StateDelta delta =
      StateDelta{x_vel_error, phi_error, R_world_from_body_error_log, x_world_error};
...
StateDot compute_qdot(const State &Q, const Controls &U, const Parameters &Z) {
  ...
  const VecNd<3> w = unit_z * std::tan(phi) * (x_vel / 1.0);
...
StateDot operator*(const double h, const StateDot &K1) {
  const StateDot anon_c70ce3 =
      StateDot{(h * (K1.a)), (h * (K1.phidot)), (h * (K1.w)), (h * (K1.vel_world))};
  return anon_c70ce3;
}
State operator+(const State &Q, const StateDot &anon_02841e) {
  const State Q2 = State{((Q.x_vel) + (anon_02841e.a)), ((Q.phi) + (anon_02841e.phidot)),
...
StateDot operator+(const StateDot &anon_c70ce3, const StateDot &anon_084b0e) {
  const StateDot anon_d080ea = StateDot{
      ((anon_c70ce3.a) + (anon_084b0e.a)), ((anon_c70ce3.phidot) + (anon_084b0e.phidot)),
      ((anon_c70ce3.w) + (anon_084b0e.w)),
      ((anon_c70ce3.vel_world) + (anon_084b0e.vel_world))};
  return anon_d080ea;
}
State rk4_integrate(const State &Q,
                    const Controls &U,
                    const Parameters &Z,
                    const double h) {
