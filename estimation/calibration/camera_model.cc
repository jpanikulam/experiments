...
Vec2 CameraModel::project(const Vec3& camera_point) const {
...
geometry::Ray CameraModel::unproject(const Vec2& image_point) const {
...
const ProjMat& CameraModel::get_k() const {
  return K_;
}

const Eigen::PartialPivLU<ProjMat>& CameraModel::get_k_inv() const {
  return K_inv_;
}
