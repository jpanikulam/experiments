...
VecNd<24> StateDelta::to_vector(const StateDelta &in_grp) {
  const VecNd<24> out =
      (VecNd<24>() << ((in_grp.accel_bias_error)[0]), ((in_grp.accel_bias_error)[1]),
       ((in_grp.accel_bias_error)[2]), ((in_grp.eps_ddot_error)[0]),
       ((in_grp.eps_ddot_error)[1]), ((in_grp.eps_ddot_error)[2]),
       ((in_grp.eps_ddot_error)[3]), ((in_grp.eps_ddot_error)[4]),
       ((in_grp.eps_ddot_error)[5]), ((in_grp.eps_dot_error)[0]),
       ((in_grp.eps_dot_error)[1]), ((in_grp.eps_dot_error)[2]),
       ((in_grp.eps_dot_error)[3]), ((in_grp.eps_dot_error)[4]),
       ((in_grp.eps_dot_error)[5]), ((in_grp.gyro_bias_error)[0]),
...
StateDelta StateDelta::from_vector(const VecNd<24> &in_vec) {
  const VecNd<6> anon_e66f16 = (VecNd<6>() << (in_vec[9]), (in_vec[10]), (in_vec[11]),
                                (in_vec[12]), (in_vec[13]), (in_vec[14]))
                                   .finished();
  const VecNd<3> anon_4e53c2 =
      (VecNd<3>() << (in_vec[0]), (in_vec[1]), (in_vec[2])).finished();
  const VecNd<6> anon_116ef6 = (VecNd<6>() << (in_vec[3]), (in_vec[4]), (in_vec[5]),
                                (in_vec[6]), (in_vec[7]), (in_vec[8]))
                                   .finished();
  const VecNd<3> anon_b3b450 =
      (VecNd<3>() << (in_vec[15]), (in_vec[16]), (in_vec[17])).finished();
  const VecNd<6> anon_105771 = (VecNd<6>() << (in_vec[18]), (in_vec[19]), (in_vec[20]),
                                (in_vec[21]), (in_vec[22]), (in_vec[23]))
                                   .finished();
  const StateDelta out =
      StateDelta{anon_4e53c2, anon_116ef6, anon_e66f16, anon_b3b450, anon_105771};
  return out;
}
...
Parameters operator+(const Parameters &a, const ParametersDelta &grp_b) {
  const Parameters out = Parameters{
      ((SE3::exp((grp_b.T_imu_from_vehicle_error_log))) * (a.T_imu_from_vehicle))};
  return out;
}
VecNd<6> Parameters::compute_delta(const Parameters &a, const Parameters &b) {
  ...
  const VecNd<6> T_imu_from_vehicle_error_log = SE3::log(T_imu_from_vehicle_error);
...
State operator+(const State &a, const StateDelta &grp_b) {
  const State out = State{
      ...
      ((a.eps_ddot) + (grp_b.eps_ddot_error)), ((a.eps_dot) + (grp_b.eps_dot_error)),
...
VecNd<24> State::compute_delta(const State &a, const State &b) {
  ...
  const VecNd<6> T_body_from_world_error_log = SE3::log(T_body_from_world_error);
  const StateDelta delta = StateDelta{accel_bias_error, eps_ddot_error, eps_dot_error,
...
VecNd<6> ParametersDelta::to_vector(const ParametersDelta &in_grp) {
  const VecNd<6> out = (VecNd<6>() << ((in_grp.T_imu_from_vehicle_error_log)[0]),
...
ParametersDelta ParametersDelta::from_vector(const VecNd<6> &in_vec) {
  const VecNd<6> anon_372276 = (VecNd<6>() << (in_vec[0]), (in_vec[1]), (in_vec[2]),
...
StateDot operator*(const double h, const StateDot &K1) {
  const StateDot anon_997f0c =
      StateDot{(h * (K1.accel_bias_dot)), (h * (K1.eps_ddot_dot)), (h * (K1.eps_ddot)),
               (h * (K1.gyro_bias_dot)), (h * (K1.eps_dot))};
  return anon_997f0c;
}
State operator+(const State &Q, const StateDot &anon_106530) {
  State Q2 = State{((Q.accel_bias) + (anon_106530.accel_bias_dot)),
