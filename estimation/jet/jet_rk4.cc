...
VecNd<24> StateDelta::to_vector(const StateDelta &in_grp) {
  const VecNd<24> out =
      (VecNd<24>() << ((in_grp.gyro_bias_error)[0]), ((in_grp.gyro_bias_error)[1]),
       ...
       ((in_grp.eps_dot_error)[5]), ((in_grp.R_world_from_body_error_log)[0]),
       ((in_grp.R_world_from_body_error_log)[1]),
       ((in_grp.R_world_from_body_error_log)[2]), ((in_grp.accel_bias_error)[0]),
       ((in_grp.accel_bias_error)[1]), ((in_grp.accel_bias_error)[2]),
       ((in_grp.x_world_error)[0]), ((in_grp.x_world_error)[1]),
       ((in_grp.x_world_error)[2]))
          .finished();
  return out;
}
StateDelta StateDelta::from_vector(const VecNd<24> &in_vec) {
  const VecNd<6> anon_b3ba1e = (VecNd<6>() << (in_vec[3]), (in_vec[4]), (in_vec[5]),
                                (in_vec[6]), (in_vec[7]), (in_vec[8]))
                                   .finished();
  const VecNd<3> anon_c76745 =
      (VecNd<3>() << (in_vec[18]), (in_vec[19]), (in_vec[20])).finished();
  const VecNd<3> anon_7c1b15 =
      (VecNd<3>() << (in_vec[21]), (in_vec[22]), (in_vec[23])).finished();
  const VecNd<3> anon_00e851 =
      (VecNd<3>() << (in_vec[0]), (in_vec[1]), (in_vec[2])).finished();
  const VecNd<6> anon_2c5be4 = (VecNd<6>() << (in_vec[9]), (in_vec[10]), (in_vec[11]),
                                (in_vec[12]), (in_vec[13]), (in_vec[14]))
                                   .finished();
  const VecNd<3> anon_a16f92 =
      (VecNd<3>() << (in_vec[15]), (in_vec[16]), (in_vec[17])).finished();
  const StateDelta out = StateDelta{anon_00e851, anon_b3ba1e, anon_2c5be4,
                                    anon_a16f92, anon_c76745, anon_7c1b15};
  return out;
}
...
GyroMeasurementDelta GyroMeasurementDelta::from_vector(const VecNd<3> &in_vec) {
  const VecNd<3> anon_e79674 =
      (VecNd<3>() << (in_vec[0]), (in_vec[1]), (in_vec[2])).finished();
  const GyroMeasurementDelta out = GyroMeasurementDelta{anon_e79674};
  return out;
}
...
State operator+(const State &a, const StateDelta &grp_b) {
  const State out =
      State{((a.gyro_bias) + (grp_b.gyro_bias_error)),
...
VecNd<24> State::compute_delta(const State &a, const State &b) {
  ...
  const StateDelta delta = StateDelta{gyro_bias_error,  eps_ddot_error,
                                      eps_dot_error,    R_world_from_body_error_log,
                                      accel_bias_error, x_world_error};
...
ParametersDelta ParametersDelta::from_vector(const VecNd<6> &in_vec) {
  const VecNd<6> anon_86a7e8 = (VecNd<6>() << (in_vec[0]), (in_vec[1]), (in_vec[2]),
                                (in_vec[3]), (in_vec[4]), (in_vec[5]))
                                   .finished();
  const ParametersDelta out = ParametersDelta{anon_86a7e8};
  return out;
}
...
AccelMeasurementDelta AccelMeasurementDelta::from_vector(const VecNd<3> &in_vec) {
  const VecNd<3> anon_7774b1 =
      (VecNd<3>() << (in_vec[0]), (in_vec[1]), (in_vec[2])).finished();
  const AccelMeasurementDelta out = AccelMeasurementDelta{anon_7774b1};
  return out;
}
StateDot compute_qdot(const State &Q, const Parameters &Z) {
  ...
  VecNd<6> eps_ddot = Q.eps_ddot;
  eps_ddot.segment<3>(3).setZero();
  ...
  const StateDot Qdot =
      StateDot{gyro_bias_dot, eps_ddot_dot, eps_ddot, w, accel_bias_dot, v};
  return Qdot;
}
StateDot operator*(const double h, const StateDot &K1) {
  const StateDot anon_967827 =
      StateDot{(h * (K1.gyro_bias_dot)),  (h * (K1.eps_ddot_dot)),
               (h * (K1.eps_ddot)),       (h * (K1.w)),
               (h * (K1.accel_bias_dot)), (h * (K1.v))};
  return anon_967827;
}
State operator+(const State &Q, const StateDot &anon_fe2e87) {
...
StateDot operator+(const StateDot &anon_967827, const StateDot &anon_a1d5a7) {
  const StateDot anon_c018a5 =
      StateDot{((anon_967827.gyro_bias_dot) + (anon_a1d5a7.gyro_bias_dot)),
               ((anon_967827.eps_ddot_dot) + (anon_a1d5a7.eps_ddot_dot)),
               ((anon_967827.eps_ddot) + (anon_a1d5a7.eps_ddot)),
               ((anon_967827.w) + (anon_a1d5a7.w)),
               ((anon_967827.accel_bias_dot) + (anon_a1d5a7.accel_bias_dot)),
               ((anon_967827.v) + (anon_a1d5a7.v))};
  return anon_c018a5;
}
