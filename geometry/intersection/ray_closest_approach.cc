...
LineApproachParameters line_ray_closest_approach(const Ray& a,
  ...
  const Ray ray_b = Ray{b.point_a, (b.point_b - b.point_a).normalized()};
  const auto approach_params = ray_ray_closest_approach(a, ray_b);

  const Vec3 pt_on_line = ray_b(approach_params.along_b);

  // Computing this here, so that one day we can make this call faster
  const double squared_distance = (a(approach_params.along_a) - pt_on_line).normSquared();
